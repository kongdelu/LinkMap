...
@property (nonatomic, strong) NSMutableArray<SymbolModel *> *allSymbols;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _allSymbols = [NSMutableArray new];
...
- (IBAction)analyze:(id)sender {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSDictionary *objMap = [self objectMapFromContent:content];
        
        NSArray <ObjectModel *>*objs = [objMap allValues];
        
        NSArray *sortedObjects = [self sortObjects:objs];
        ...
        if (1 == groupButtonState) {
            [self buildCombinationResultWithObjects:sortedObjects];
        } else {
            [self buildResultWithObjects:sortedObjects];
        }
        NSArray *sortedAllSymbols = [[self sortAllSymbols:_allSymbols] subarrayWithRange:NSMakeRange(0, 100)];
        NSLog(@"sortedAllSymbols: %@", sortedAllSymbols);
        
        [_result appendFormat:@"%@", sortedAllSymbols];
...
- (NSMutableDictionary *)objectMapFromContent:(NSString *)content {
    NSMutableDictionary <NSString *,ObjectModel *>*objectMap = [NSMutableDictionary new];
    // 符号文件列表
        ...
        if([line hasPrefix:@"#"]) {
            ...
            if(reachFiles == YES && reachSections == NO && reachSymbols == NO) {
                ...
                if(range.location != NSNotFound) {
                    ObjectModel *obj = [ObjectModel new];
                    obj.file = [line substringFromIndex:range.location+1];
...
- (NSArray *)sortAllSymbols:(NSArray *)symbols {
    NSArray *sortedSymbols = [symbols sortedArrayUsingComparator:^NSComparisonResult(SymbolModel *  _Nonnull sym1, ObjectModel *  _Nonnull sym2) {
...
- (NSArray *)sortObjects:(NSArray *)objects {
    NSArray *sortedSymbols = [objects sortedArrayUsingComparator:^NSComparisonResult(ObjectModel *  _Nonnull obj1, ObjectModel *  _Nonnull obj2) {
...
- (void)buildResultWithObjects:(NSArray *)symbols {
    ...
    for(ObjectModel *symbol in symbols) {
...
- (void)buildCombinationResultWithObjects:(NSArray *)symbols {
    self.result = [@"库大小\t库名称\r\n\r\n" mutableCopy];
    NSUInteger totalSize = 0;
    ...
    for(ObjectModel *symbol in symbols) {
        ...
        if ([name hasSuffix:@")"] &&
            ...
            ObjectModel *combinationSymbol = [combinationMap objectForKey:component];
            if (!combinationSymbol) {
                combinationSymbol = [[ObjectModel alloc] init];
    ...
    NSArray <ObjectModel *>*combinationSymbols = [combinationMap allValues];
    
    NSArray *sortedSymbols = [self sortObjects:combinationSymbols];
    
    NSString *searchKey = _searchField.stringValue;
    
    for(ObjectModel *symbol in sortedSymbols) {
        if (searchKey.length > 0) {
            if ([symbol.file containsString:searchKey]) {
                [self appendResultWithSymbol:symbol];
                totalSize += symbol.size;
...
- (void)appendResultWithSymbol:(ObjectModel *)model {
    NSString *size = nil;
